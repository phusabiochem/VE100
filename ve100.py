...
#             menu_labelframe.place_forget()
			...
			control0_labelframe = LabelFrame(viewresults_labelframe, bg=FRAME_BACKGROUND_COLOR_1, width=150, height=70)
			control0_labelframe.place(x=18, y=340)
			control1_labelframe = LabelFrame(viewresults_labelframe, bg=FRAME_BACKGROUND_COLOR_1, width=130, height=70)
			control1_labelframe.place(x=252,y=340)
			control2_labelframe = LabelFrame(viewresults_labelframe, bg=FRAME_BACKGROUND_COLOR_1, width=289, height=70)
			control2_labelframe.place(x=380, y=340)
			control3_labelframe = LabelFrame(viewresults_labelframe, bg=FRAME_BACKGROUND_COLOR_1, width=120, height=70)
			control3_labelframe.place(x=669, y=340)
...
#                     final_img.save(name+'.png', 'png')
			...
			firstband_label = Label(control0_labelframe, text="First band", fg ="dodger blue", font=('Courier', 10))
			firstband_label.grid(row=0, column=0, padx=17, pady=3, sticky=S) 
			lastband_label = Label(control0_labelframe, text="Last band ", fg ="red", font=('Courier', 10))
			lastband_label.grid(row=1, column=0, padx=17, pady=3) 
			
			firstband_entry = Entry(control0_labelframe, justify='right', width=6, font=('Courier',15))
			firstband_entry.grid(row=0, column=1, padx=17, pady=3)
			firstband_entry.insert(0, 2000)
			lastband_entry = Entry(control0_labelframe, justify='right', width=6, font=('Courier',15))
			lastband_entry.grid(row=1, column=1, padx=17, pady=3)
			lastband_entry.insert(0, 100)
			
			open_button = Button(control1_labelframe, text='Open', font=('Courier', 13), fg=BUTTON_TEXT_COLOR_1, bg=BUTTON_BACKGROUND_COLOR_2, borderwidth=2, width=8, height=2, command=open_clicked)
			open_button.pack(padx=7, pady=7)

			find_button = Button(control2_labelframe, state='disable', text='Find', font=('Courier', 13), fg=BUTTON_TEXT_COLOR_1, bg=BUTTON_BACKGROUND_COLOR_2, borderwidth=2, width=6, height=2, command=find_clicked)
			find_button.place(x=170,y=7)
			save_button = Button(control3_labelframe, state='disable',text='Save', font=('Courier', 13), fg=BUTTON_TEXT_COLOR_1, bg=BUTTON_BACKGROUND_COLOR_2, borderwidth=2, width=6, height=2, command=save_clicked)
			save_button.pack(padx=7, pady=7)
...
#                         wb.save(path_name + 'result.xlsx')
			...
			cam_mode1_button = Button(cam_labelframe, bg='lawn green', bd=1, text='MODE 1',font=("Courier",10,'bold'), command=cam_mode1_clicked)
			cam_mode1_button.pack(padx=1, pady=2, side=LEFT)
			cam_mode2_button = Button(cam_labelframe, bg='grey75', bd=1, text='MODE 2',font=("Courier",10,'bold'), command=cam_mode2_clicked)
	...
	#                         wb.save(path_name + 'result.xlsx')
					...
					if(running==1):
						...
						GPIO.output(RELAY_PIN, GPIO.HIGH)
						camera_preview()
			capture_button = Button(button_labelframe, bg=BUTTON_BACKGROUND_COLOR_3, bd=1, text='CAPTURE',font=("Courier",13,'bold'), width=15, height=2, command=capture_click)
			# ~ capture_button.place(x=210,y=3)
			capture_button.pack(padx=1, pady=2, side=LEFT)
			stop_button = Button(button_labelframe, bg=BUTTON_BACKGROUND_COLOR_4, bd=1, text='STOP',font=("Courier",13,'bold'), width=15, height=2, command =stop_click)
			# ~ stop_button.place(x=427,y=3)
			stop_button.pack(padx=1, pady=2, side=LEFT)
...
#                         wb.save(path_name + 'result.xlsx')
			...
			para_labelframe = LabelFrame(button_labelframe, bg=FRAME_BACKGROUND_COLOR_1, width=80, height=55)
			# ~ para_labelframe.place(x=3,y=4)
			para_labelframe.pack(padx=2, pady=2, side=LEFT)
			present_voltage_label = Label(para_labelframe, font=('Helvetica',10,'bold'), fg='black', bg=FRAME_BACKGROUND_COLOR_1)
			present_voltage_label.pack(padx=2, pady=2)
			present_current_label = Label(para_labelframe, font=('Helvetica',9,'bold'), fg='black', bg='red')
			present_current_label.pack(padx=2, pady=2)
