...
#                         wb.save(path_name + 'result.xlsx')
			...
			cam_labelframe.pack(padx=2, pady=2, side=LEFT)
			cam_mode_combobox = ttk.Combobox(cam_labelframe, state='readonly', width=6, textvariable = n)
			cam_mode_combobox['values'] = ('Mode 1', 'Mode 2')
			cam_mode_combobox.pack(padx=1, pady=1, side=LEFT, anchor=S)
			cam_mode_combobox.current(0)
			
			def cam_update_button_clicked():
				global camera	
				
				camera.stop_preview()
				if(cam_mode_combobox.current() == 0):
					camera.framerate = 1
			...
			cam_update_button = Button(cam_labelframe, bg=BUTTON_BACKGROUND_COLOR_1, bd=1, text='UPDATE',font=("Courier",13,'bold'), width=5, height=2, command=cam_update_button_clicked)
			cam_update_button.pack(padx=2, pady=2, side=LEFT)
